...
async def summarize_image(image_path: str) -> str:
    ...
    prompt = (
        "你是校园墙的审核助理, 需要帮助管理员几秒内了解投稿图片的主旨。"
        "请先识别图片中最重要的人物、事件、物品和文字, 理解他们表达的意思。"
        "用一句不超过40个汉字的中文总结投稿的重点、目的或行动, 让读者无需打开图片也能了解核心信息。"
        "严禁逐字抄写或分段列出文字, 只保留最关键的事实或结论, 不要添加前缀、表情或推测图片外的信息。"
    )
